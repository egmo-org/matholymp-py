...
def get_num_contestants_per_team(db):
    """Return the number of contestants per team at this event."""
    return get_config_var_int(db, 'MATHOLYMP_NUM_CONTESTANTS_PER_TEAM')
...
def country_invitation_letter_register(db):
    """
    Return whether registering users can generate invitation letters
    for their country (combined for all participants).
    """
    return get_config_var_bool(db,
                               'MATHOLYMP_COUNTRY_INVITATION_LETTER_REGISTER')
